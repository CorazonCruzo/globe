...
3. Для каждого GeoJSON feature:
   ...
   - **Антимеридиан (±180°)**: для полигонов, пересекающих антимеридиан (например, Россия, Фиджи), проверяем разницу долгот соседних вершин. Если > 180°, разрезаем полигон на две части по антимеридиану
   - **Subdivision длинных рёбер**: рёбра длиннее ~5° подразделяются на сегменты с интерполяцией по great circle arc, чтобы линии следовали кривизне сферы
4. Проекция 2D координат на сферу: `x = R * cos(lat) * cos(lon)`, `y = R * sin(lat)`, `z = -R * cos(lat) * sin(lon)`
5. **Z-fighting offset**: все country-меши рендерятся на радиусе `R + 0.001` (чуть выше поверхности океана), чтобы избежать z-fighting с GlobeFeature (океан на радиусе R)
6. Нормали пересчитываются после проекции через `geometry.computeVertexNormals()`
